        ...
        for numberOfFineTuningEpochs in fineTuningEpochList:
            for amountOfGestures in fineTuningGestureList:
                for amountOfRepetitions in fineTuningRepetitionList:
                    ...
                    # 8*5 * 13 (520) would work pretty well on TPU both for training and validation as well
                    ###################################################################################
                    # seq_len = 150
                    # stride = 53
                    # mean = 3
                    # -> allNumOfFineTuningSamples is a multiple of 17
                    # 8*10 * 17 (1360) would work pretty well on TPU both for training and validation as well
                    ###################################################################################
                    # seq_len = 150
                    # stride = 10
                    # mean = 21
                    # -> allNumOfFineTuningSamples is a multiple of 84
                    # 10 * 84 (840) would work pretty well on TPU both for training and validation as well
                    ###################################################################################
                    #if amountOfGestures * amountOfRepetitions >= 84: # this is dynamic (20 for: sl 40, 51 for sl 150)
                    batchSize = 520
                    #else:
                    #    batchSize = amountOfGestures * amountOfRepetitions * 84
                    ...
                    if validation == 'inter-subject':
                        ...
                        workingDirectory = baseDrive+'data/inter-subject/dbb/'
                        trainingUsers = list(range(11, 21))
                    ...
                    if validation == 'inter-subject-split':
                        directory = baseDrive+"data/dbb"
                        workingDirectory = baseDrive+'data/inter-subject-split/dbb/'
                        trainingUsers = list(range(11, 21))
                        for testUser in trainingUsers: # LOSOCV for inter-subject validation
                            testUsers = [testUser]
                            if training == 'pre-training':
                                ...
                                adaptModel(fineTuneUsers=fineTuneUsers,
                                    ...
                                    number_of_classes=number_of_classes)
                    elif validation == 'inter-session':
                        allNumOfFineTuningSamples = 10 * amountOfGestures * amountOfRepetitions * ((1000-mean+1-seq_len)//stride + 1)
                        fineTuningStepsPerEpoch = allNumOfFineTuningSamples // batchSize
                        ...
                        trainingStepsPerEpoch = allNumOfTrainingSamples // batchSize
                        directory = baseDrive+"data/dbb"
                        workingDirectory = baseDrive+'data/inter-session/dbb/'
                        trainingUsers = list(range(1, 11))
                        testUsers = list(range(11, 21))
                        fineTuneUsers = list(range(11, 21))
                        testUser='2ndSession'
                        if training == 'pre-training':
                        ...
                        testUsers = list(range(11, 21))
                        directory = baseDrive+"data/dbb"
                        workingDirectory = baseDrive+'data/inter-session-split/dbb/'
                        testUser='2ndSession'
                        if training == 'pre-training':
                            allNumOfTrainingSamples = 10 * 8 * 10 * ((1000-mean+1-seq_len)//stride + 1)
                            trainingStepsPerEpoch = allNumOfTrainingSamples // batchSize
                            ...
                            fineTuneUsers = list(range(11, 21))
                            allNumOfFineTuningSamples = 10 * amountOfGestures * 5 * ((1000-mean+1-seq_len)//stride + 1)
                        ...
                        allNumOfValidationSamples = 10 * 8 * 5 * ((1000-mean+1-seq_len)//stride + 1)
                        validationStepsPerEpoch = allNumOfValidationSamples // batchSize
                        allNumOfTrainingSamples = 10 * 8 * 5 * ((1000-mean+1-seq_len)//stride + 1)
                        trainingStepsPerEpoch = allNumOfTrainingSamples // batchSize
                        directory = baseDrive+"data/dbb"
                        workingDirectory = baseDrive+'data/intra-session/dbb/'
                        trainingUsers = list(range(11, 21))
                        testUsers = list(range(11, 21))
                        trainingRepetitions = [1, 3, 5, 7, 9]
                        testRepetitions = [2, 4, 6, 8, 10]
                        ...
                        trainingStepsPerEpoch = allNumOfTrainingSamples // batchSize
                        directory = baseDrive+"data/dbb"
                        workingDirectory = baseDrive+'data/intra-session-separated/dbb/'
                        trainingUsers = list(range(11, 21))
...
generalScenario(validation='inter-session-split', training='pre-training')
generalScenario(validation='inter-subject-split', training='fine-tuning')
generalScenario(validation='inter-session-split', training='fine-tuning')